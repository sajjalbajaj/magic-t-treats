...
| **Auth is never on the public path, and every auth call is bounded** | `proxy.ts` returns before any network work unless the path is `/admin` or `/auth/login`, and both it and `getAdminSession()` race `getUser()` against a 3s timeout, failing closed. On 2026-08-28 the project's auth service stopped responding while the database and storage stayed healthy; because the middleware awaited `getUser()` on *every* request with no ceiling, the entire public site hung. Public pages read from the cookie-free client and have no session to use, so the call bought nothing. |
